...
function displayFiles() {
	...
	$.getJSON(url, function(data) {
		...
		$.each(data, function(i, item) {
			...
			file += '<a href="#" title="' + item.filename + '" class="download">' + item.filename + '</a>';
			if (item.deletable == true) {
				file += '&nbsp;&nbsp;<a href="#" title="' + item.filename + '">Delete</a>';
			}
			file += '</div>';
...
$(document).ready(function() {
	...
	$("#fileForm").submit(function(event) {
		...
		$.ajax({
			...
			success : function() {
				displayFiles();
				$('#fileForm input[name*=attach]').val('');
			},
			error : function(error) {
				alert("Error");
				console.log(error);
...
$(document).on('click', '#all-files', function(e) {
 	if ($(e.target).is('a.download')) {
		...
		var url = $('#deleteFileForm').attr('action');
		var $filename = $(e.target).attr('title');
		url += $filename;
...
<div id="wrap">
	...
	<sf:form id="fileForm" action="/upload" method="post" enctype="multipart/form-data">
		<div><input type="file" name="attach" /><input type="submit" value="Submit" /></div>
	</sf:form>
